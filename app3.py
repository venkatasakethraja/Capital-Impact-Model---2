...
st.markdown(
    "*BoE-aligned CET1 presentation across the selected conflict duration: absolute CET1 bridge and CET1 ratio bridge*"
)
st.markdown("---")
...
# -----------------------------
# Interpret the scenario over the entire selected conflict duration.
duration_factor = clamp(0.55 + 0.45 * duration_years, 0.45, 1.90)
...
macro_multiplier = 1 + 0.04 * severity + 0.06 * duration_years


# -----------------------------
# BoE-aligned decomposition over conflict duration
# -----------------------------
# Income lines scale with duration, but with stress compression.
baseline_nii = base_rwa * annual_nii_to_rwa * duration_years
nii_rate_factor = clamp(
    1 + (yield_shock_bps / 10000) * 4.5 * p["nii_sensitivity"] - 0.035 * severity - 0.02 * max(0.0, duration_years - 1.0),
    0.55,
    1.45,
)
net_interest_income = baseline_nii * nii_rate_factor

baseline_fee_income = base_rwa * annual_fee_to_rwa * duration_years * p["fee_resilience"]
fee_stress_factor = clamp(1 - (0.06 * severity + 0.04 * max(0.0, duration_years - 1.0)), 0.35, 0.95)
net_fee_and_commission_income = baseline_fee_income * fee_stress_factor

baseline_traded_income = base_rwa * annual_traded_income_to_rwa * duration_years * p["trading_income_resilience"]
traded_income_factor = clamp(
    1 + 0.04 * severity - 0.03 * max(0.0, duration_years - 1.0) - 0.03 * sanctions_exposure,
    0.25,
    1.80,
)
net_traded_income = baseline_traded_income * traded_income_factor

operating_expenses = base_rwa * annual_opex_to_rwa * duration_years * clamp(0.95 + 0.05 * severity + 0.03 * max(0.0, duration_years - 1.0), 0.95, 1.60)
...
stressed_pd = clamp(base_pd * pd_multiplier * sector_overlay, base_pd, 0.12)
stressed_lgd = clamp(
    downturn_lgd * (1 + 0.05 * severity / 10 + 0.06 * max(0.0, duration_years - 1.0)),
    downturn_lgd,
    0.70,
)
...
misconduct = base_rwa * misconduct_cost_rate * duration_years * (0.5 + 0.5 * severity / 10) if include_misconduct_costs else 0.0
operational_risk = total_assets * operational_risk_rate_bps * cyber_vulnerability * duration_years * clamp(0.7 + 0.08 * severity + 0.05 * max(0.0, duration_years - 1.0), 0.7, 1.8)
losses_total = impairments + traded_risk_losses + misconduct + operational_risk

# RWA + other capital items
credit_rwa_inflation = loan_book * p["loan_rwa_density"] * (0.010 + 0.0035 * severity) * (0.85 + 0.25 * min(duration_years, 3.0))
market_rwa_inflation = trading_book * 0.20 * (0.010 + 0.003 * severity)
fx_rwa_inflation = base_rwa * 0.0008 * fx_stress_index
operational_rwa_inflation = base_rwa * 0.0006 * severity * cyber_vulnerability * min(duration_years, 2.5)
...
# Tax / other capital
# Includes sanctions / corridor losses and other capital drag across the conflict duration.
direct_exposure_loss = direct_exposure_base * sanctions_exposure * (
    0.015 * severity + 0.012 * max(duration_years, 0.25)
)
tax_and_other_pl_and_capital = (base_rwa * tax_and_other_pl_rate * duration_years) - direct_exposure_loss
...
stress_low_point_pre_sma = 100.0 * capital_pre_sma / stressed_rwa_pre_sma if stressed_rwa_pre_sma > 0 else 0.0
drawdown_pre_sma = stress_low_point_pre_sma - opening_ratio_check

# SMAs
pre_sma_drawdown_abs = max(0.0, opening_ratio_check - stress_low_point_pre_sma)
...
stress_low_point_post_sma = 100.0 * capital_post_sma / stressed_rwa_post_sma if stressed_rwa_post_sma > 0 else 0.0
drawdown_post_sma = stress_low_point_post_sma - opening_ratio_check

stressed_cet1 = capital_post_sma
stressed_cet1_ratio = stress_low_point_post_sma
...
smas_rp = stress_low_point_post_sma - stress_low_point_pre_sma
...
for idx, value in enumerate(ratio_values):
    if idx in [0, 5, 10, 14, 15, 17, 18]:
        ratio_text.append(f"{value:.1f}%")
    else:
        ratio_text.append(f"{value:+.1f}pp")
...
capital_values = [
    ...
    net_traded_income,
    -operating_expenses,
    starting_cet1 + net_income_less_expenses,
    -impairments,
    -traded_risk_losses,
    -misconduct,
    -operational_risk,
    capital_before_rwa_other,
    tax_and_other_pl_and_capital,
    -distributions,
    capital_pre_sma,
    sma_capital_support,
    capital_post_sma,
...
capital_text = []
for idx, value in enumerate(capital_values):
    if idx in [0, 5, 10, 13, 15]:
        capital_text.append(usd_billions(value))
    else:
        capital_text.append(usd_billions(value))
...
bridge_df = pd.DataFrame(
    {
        "Line item": ratio_labels,
        "CET1 capital ratio": ratio_text,
    }
)
...
summary_df = pd.DataFrame(
    {
        ...
        "Value": [
            ...
            pp(net_income_less_expenses_rp),
            pp(losses_rp),
            pp(rwas_other_capital_rp),
            pp(drawdown_pre_sma),
            pp(smas_rp),
            pp(drawdown_post_sma),
            f"{stress_low_point_post_sma:.1f}%",
...
mapping_df = pd.DataFrame(
    {
        ...
        "Scenario mapping": [
            "Core interest earnings across the selected conflict duration",
            "Fee income under stressed client activity and volumes",
            "Trading / markets income across the conflict period",
            "Recurring expense base across the conflict period",
            "Credit losses from PD/LGD stress",
            "IRRBB and fair-value / trading losses net of hedging",
...
# Chart builders
# -----------------------------
def make_waterfall(labels, values, measure, text, title, yaxis_title):
    fig = go.Figure(
        go.Waterfall(
            orientation="v",
            measure=measure,
            ...
            connector={"line": {"color": "rgba(180,180,180,0.55)", "width": 1}},
            decreasing={"marker": {"color": "#FF5A52", "line": {"color": "#FF5A52", "width": 1}}},
            increasing={"marker": {"color": "#FFE600", "line": {"color": "#FFE600", "width": 1}}},
            totals={"marker": {"color": "#FFD400", "line": {"color": "#FFD400", "width": 1}}},
            cliponaxis=False,
        )
    )
    fig.update_layout(
...
with tab1:
    ...
    m4.metric("Conflict Duration", f"{duration_months} months")
    m5.metric("Stress low-point ratio", f"{stress_low_point_post_sma:.1f}%", f"{drawdown_post_sma:.1f}pp", delta_color="inverse")

    st.markdown("---")

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Starting CET1", usd_billions(starting_cet1))
    c2.metric("Stress low-point CET1", usd_billions(capital_post_sma), usd_billions(capital_post_sma - starting_cet1))
    c3.metric("Starting CET1 ratio", f"{opening_ratio_check:.1f}%")
    c4.metric("Breach indicator", f"{breach_indicator:.1f}%", delta_color="inverse")

    c5, c6, c7, c8 = st.columns(4)
    c5.metric("Net income less expenses", pp(net_income_less_expenses_rp))
    c6.metric("Losses", pp(losses_rp))
    c7.metric("RWA + other capital items", pp(rwas_other_capital_rp))
    c8.metric("SMAs", pp(smas_rp))
    ...
    st.plotly_chart(
        make_waterfall(
            ...
            capital_measure,
            capital_text,
            "Absolute CET1 Capital Bridge Across Conflict Duration",
            "CET1 capital ($B)",
        ),
    ...
    st.plotly_chart(
        make_waterfall(
            ...
            ratio_measure,
            ratio_text,
            "CET1 Ratio Bridge Across Conflict Duration",
            "CET1 ratio / percentage-point contribution",
        ),
        use_container_width=True,
    )
    ...
    with st.expander("Live Calculation Audit Log"):
        st.markdown(
            ...
            **Opening balance sheet**
            ...
            - Starting CET1 ratio = **{baseline_cet1_ratio:.1f}%**
            ...
            **RWA + other capital items**
            ...
            - RWA impact line = **{rwas_leverage_exposure:+.2f}pp**
            - Tax / other capital = **{usd_billions(tax_and_other_pl_and_capital)}**
    ...
    gross_component_df = pd.DataFrame(
        {
            "Component": [
                ...
                "Distributions",
                "Stress low-point CET1",
                "Stress low-point ratio",
            ],
            "Value": [
                usd_billions(net_interest_income),
                usd_billions(net_fee_and_commission_income),
                usd_billions(net_traded_income),
                usd_billions(-operating_expenses),
                usd_billions(-impairments),
                usd_billions(-traded_risk_losses),
                usd_billions(-misconduct),
                usd_billions(-operational_risk),
                usd_billions(-direct_exposure_loss),
                usd_billions(-distributions),
                usd_billions(capital_post_sma),
                f"{stress_low_point_post_sma:.1f}%",
            ],
        }
    )
    st.markdown("### Underlying Diagnostics")
...
with tab3:
    ...
    st.markdown(
        """
        This version reflects the full selected conflict duration rather than using Year 0 / Year 1 language.

        The presentation keeps the BoE-style sequencing because it is intuitive for senior banking audiences:
        - start point
        - income lines
        ...
        - **RWA + other capital items** subtotal
        - **Drawdown pre-SMA**
        - **SMAs**
        - **Drawdown post-SMA**
        - **Stress low-point**

        In addition, this version now shows the same story in two ways:
        - **absolute CET1 capital bridge ($B)**
        - **CET1 ratio bridge (percentage points)**
        """
    )
    ...
    st.latex(r"Stress\ low\ point_{pre\text{-}SMA} = Start + Net\ income\ less\ expenses - Losses + RWAs\ and\ other\ capital\ items")
    st.latex(r"Stress\ low\ point_{post\text{-}SMA} = Stress\ low\ point_{pre\text{-}SMA} + SMA")

    st.markdown(
        """
        **Duration treatment**
        - income and recurring cost items scale with conflict duration
        - impairment and risk factors rise with severity and persistence
        - RWA add-ons are cumulative but capped from becoming mechanically explosive

        **Presentation choice**
        The ratio waterfall follows supervisory-style logic, while the capital waterfall gives users the same story in absolute dollar terms.
        """
    )
...
with tab4:
    ...
    st.markdown(
        """
        #### Why these changes improve the model
        - Absolute CET1 and CET1 ratio are now both visible.
        - The visuals are cleaner and more readable for client presentation.
        - Labels no longer imply a one-year stress when the scenario duration can run from 1 to 36 months.

        #### What remains aligned to supervisory-style presentation
        - income, losses, denominator/capital items, and management actions are still clearly separated
        - drawdown pre-SMA and post-SMA remain explicit
        - the ratio chart is still expressed in percentage-point contributions

        #### Limits
        - This remains an advisory model, not a regulatory capital engine.
        - SMA mechanics are stylized management actions.
        - Tax and other P&L / capital is still a simplified bucket.
...
with tab5:
    ...
    st.markdown(
        """
        > **"Can we show the same stress both in dollars and in capital-ratio terms?"**
        - Yes. The dashboard now shows both an absolute CET1 bridge and a CET1 ratio bridge.

        > **"Are we still presenting this in a supervisory-style way?"**
        - Yes. The structure remains aligned to the BoE-style sequencing of income, losses, RWA/capital items, and SMAs.

        > **"Does the chart now reflect the full conflict duration?"**
        - Yes. The wording and scaling now refer to the selected conflict duration rather than forcing a Year 1 framing.

        > **"Can we still explain where sanctions and cyber show up?"**
        - Yes. Sanctions remain in tax and other P&L / capital, and cyber / disruption remains in operational risk.
        """
    )
